...
- Each invocation receives exactly one task ID from the orchestrator; implement only that task (respect `[P]` markers if relevant) and exit immediately after reporting DONE/BLOCKED/FAILED for it. Never invent tasks or modify scope.
...
1. Pre-flight
   ...
   - Confirm the orchestrator-provided task ID exists, is unchecked, and matches the description supplied in the brief. If mismatched, halt and return BLOCKED.
2. Parse the relevant phase/context for the assigned task (Setup, Foundational, USx, Polish). Document dependencies and `[P]` groupings before editing files.
3. Execute the assigned task only:
   - Record `started_at`.
   ...
   - Mark the task checkbox to `[x]` on success. Emit a JSON log entry with status, files edited, commands run, stdout/stderr snippets (≤2 KB).
   - Do not proceed to additional tasks even if they remain unchecked; return control to the orchestrator with the JSON/status payload.
...
Embedded Skills (must follow exactly):
...
Completion:
- After the assigned task is DONE/BLOCKED/FAILED, emit the summary JSON for that single task (match the schema described in `agents/orchestrator.md`) including timestamps, commands, and next action (`READY_FOR_REVIEW` vs `Await clarification`).
