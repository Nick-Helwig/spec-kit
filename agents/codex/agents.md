...
6. **Timeout hygiene**  
   The orchestrator delegation relies on `.codex/config.toml` containing `startup_timeout_sec = 120` and `tool_timeout_sec = 300` under `[mcp_servers.subagents]`. If you clone a fresh repo and see 60 s MCP timeouts, rerun `specify init --ai codex` or manually edit the config.
