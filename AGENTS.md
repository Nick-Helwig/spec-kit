...
- Run `.codex/scripts/bootstrap-subagents.sh` (or `.codex/scripts/bootstrap-subagents.ps1`) once per machine to install the Codex sub-agent MCP server before invoking any `/speckit.*` commands inside Codex.
